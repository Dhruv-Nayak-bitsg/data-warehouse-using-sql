/* ==============================================================================
   DDL SCRIPT: GOLD LAYER VIEWS
   ------------------------------------------------------------------------------
   Purpose:   Constructs the final dimensional model (Star Schema) consisting
              of Dimensions and Facts. These views sit on top of the Silver 
              layer to provide a clean, business-ready, analytical dataset.
   Usage:     These views are the primary source for BI, reporting, and analytics.
============================================================================== */

-- ==============================================================================
-- 1.0 Dimension: gold.dim_customers
-- ==============================================================================
DROP VIEW IF EXISTS gold.dim_customers;
GO

CREATE VIEW gold.dim_customers AS
SELECT
    ROW_NUMBER() OVER (ORDER BY cust.cst_id) AS customer_key, 
    cust.cst_id                              AS customer_id,
    cust.cst_key                             AS customer_number,
    cust.cst_firstname                       AS first_name,
    cust.cst_lastname                        AS last_name,
    loc.cntry                                AS country,
    cust.cst_marital_status                  AS marital_status,
    -- Prioritize CRM gender, fallback to ERP demographic data if unavailable
    CASE 
        WHEN cust.cst_gndr <> 'n/a' THEN cust.cst_gndr 
        ELSE ISNULL(demo.gen, 'n/a')  			   
    END                                      AS gender,
    demo.bdate                               AS birthdate,
    cust.cst_create_date                     AS create_date
FROM silver.crm_cust_info AS cust
LEFT JOIN silver.erp_cust_az12 AS demo
    ON cust.cst_key = demo.cid
LEFT JOIN silver.erp_loc_a101 AS loc
    ON cust.cst_key = loc.cid;
GO

-- ==============================================================================
-- 2.0 Dimension: gold.dim_products
-- ==============================================================================
DROP VIEW IF EXISTS gold.dim_products;
GO

CREATE VIEW gold.dim_products AS
SELECT
    ROW_NUMBER() OVER (ORDER BY prd.prd_start_dt, prd.prd_key) AS product_key, 
    prd.prd_id                                                 AS product_id,
    prd.prd_key                                                AS product_number,
    prd.prd_nm                                                 AS product_name,
    prd.cat_id                                                 AS category_id,
    cat.cat                                                    AS category,
    cat.subcat                                                 AS subcategory,
    cat.maintenance                                            AS maintenance,
    prd.prd_cost                                               AS cost,
    prd.prd_line                                               AS product_line,
    prd.prd_start_dt                                           AS start_date
FROM silver.crm_prd_info AS prd
LEFT JOIN silver.erp_px_cat_g1v2 AS cat
    ON prd.cat_id = cat.id
WHERE prd.prd_end_dt IS NULL; -- Ensures only active, current products are included
GO

-- ==============================================================================
-- 3.0 Fact: gold.fact_sales
-- ==============================================================================
DROP VIEW IF EXISTS gold.fact_sales;
GO

CREATE VIEW gold.fact_sales AS
SELECT
    sls.sls_ord_num     AS order_number,
    dp.product_key      AS product_key,
    dc.customer_key     AS customer_key,
    sls.sls_order_dt    AS order_date,
    sls.sls_ship_dt     AS shipping_date,
    sls.sls_due_dt      AS due_date,
    sls.sls_sales       AS sales_amount,
    sls.sls_quantity    AS quantity,
    sls.sls_price       AS price
FROM silver.crm_sales_details AS sls
LEFT JOIN gold.dim_products AS dp
    ON sls.sls_prd_key = dp.product_number
LEFT JOIN gold.dim_customers AS dc
    ON sls.sls_cust_id = dc.customer_id;
GO
